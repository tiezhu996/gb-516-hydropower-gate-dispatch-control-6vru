...
- 每条指令必须录入调度许可四项条件：许可开始时间、许可结束时间、最低水位、最高水位；建单（及草稿编辑）时校验开始时间早于结束时间、最低水位不高于最高水位。
- 复核通过后开工（`approved → executing`）时按目标闸门的 `relatedCode` 读取所属库区当前水位（`Reservoir.metricValue`），只有当前时间落在许可时段内且水位处于 `[最低水位, 最高水位]` 区间才允许动作；条件不满足返回 422 `permit_window`，指令与闸门保持原状态。
