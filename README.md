...
- 每条指令强制录入四项调度许可条件：许可开始时间、许可结束时间、最低水位、最高水位；建单（含草稿改单）校验许可时段先后与水位上下限。
- 复核通过后开工（`approved → executing`）时按目标闸门关联的库区实时读取当前水位；仅当当前时间落在许可时段且水位处于上下限之间才允许动作，否则拒绝执行，指令版本与闸门状态保持原样，错误信息注明越界的时段或水位。
