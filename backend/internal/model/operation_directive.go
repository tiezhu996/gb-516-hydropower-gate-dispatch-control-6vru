...
type OperationDirective struct {
	...
	// PermitStartAt/PermitEndAt bound the dispatch permit window; gate action is
	// only allowed while the execution instant falls within it. MinWaterLevel and
	// MaxWaterLevel constrain the linked reservoir's current level (same unit as
	// the reservoir metric) at execution time.
	PermitStartAt time.Time           `json:"permitStartAt" gorm:"index"`
	PermitEndAt   time.Time           `json:"permitEndAt" gorm:"index"`
